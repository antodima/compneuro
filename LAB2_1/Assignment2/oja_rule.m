...
for t=1:n_epochs
    ...
    for n=1:size(U,2)       % loop over the dataset patterns
        u = U(:,n);
        v = W' * u;
        dw = (v * u) - (alpha * v^2 * W);   % Oja delta
        W = W + lr * dw;                    % Oja learning rule 
    end;
    ...
    fprintf('Epoch %d/%d, |W|=%5.10f, delta norm=%5.10f, norm thr=%5.10f, 1/α=%0.5f \n',t,n_epochs,norm_w,delta_norm,delta_norm_thr,(1/alpha));
